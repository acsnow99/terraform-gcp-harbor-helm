...
CMD cd /minecraft && ./bedrock_server && mkdir /worlds-backup/'"$WORLDNAME"' && chmod 700 /auto-backup.sh && /auto-backup.sh' > ./k8s-minecraft-image/Dockerfile
# tail -f /dev/null
...
sudo docker push --no-cache alexcraigs/k8s-minecraft-new-world:"${worldname}"
